...
SUBROUTINE matrixAPrint(A,A11,A12,A13,A21,A22,A23,A31,A32,A33)
  ...
  do i=0,(6*Ndr)+5
    write(11, '(1600F17.8)')( Real(A(i+1,j+1)) ,j=0,(6*Ndr)+5)
  end do
  close(11)

  open(unit=24, file='M/graphLHSImag.txt', ACTION="write", STATUS="replace")
  do i=0,(6*Ndr)+5
    write(24, '(1600F17.8)')( aImag(A(i+1,j+1)) ,j=0,(6*Ndr)+5)
  end do
  close(24)
...
SUBROUTINE matrixBPrint(B,B11,B12,B13,B21,B22,B23,B31,B32,B33)
  ...
  do i=0,(6*Ndr)+5
    write(12, '(1600F17.8)')( Real(B(i+1,j+1)) ,j=0,(6*Ndr)+5)
  end do
  close(12)

  open(unit=142, file='M/graphRHSImag.txt', ACTION="write", STATUS="replace")
  do i=0,(6*Ndr)+5
    write(142, '(1600F17.8)')( aImag(B(i+1,j+1)) ,j=0,(6*Ndr)+5)
  end do
  close(142)
