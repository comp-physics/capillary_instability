!3d pure viscous NO INERTIA solver!
...
!FLOW PARAMETERS
    Real             GAM,RHO,MU,NU,aa
    PARAMETER        (GAM=1.0,RHO=1.0,MU=0.5,NU=MU/RHO)
...
SUBROUTINE evsolve
  ...
  Real, allocatable, dimension(:,:,:)  :: xDddr,x2Dddr
  ...
  call buildA(A,xDddr,x2Dddr)
  call buildB(B,xDddr,x2Dddr)
...
SUBROUTINE buildA(A,xDddr,x2Dddr)
  ...
  A11(:,:) = NU*(x2Dddr(:,:,2)+xDddr(:,:,1)-Dddr(:,:,0)-((K**2)*x2Dddr(:,:,0)))

      !BC for stress condition at r=aa

      A11(Ndr,:) = Dddr(Ndr,:,0)*GAM*((K**2)-1./(aa**2))
  ...
  A22(:,:) = NU*(xDddr(:,:,2)+Dddr(:,:,1)-((K**2)*(xDddr(:,:,0))))
...
SUBROUTINE buildB(B,xDddr,x2Dddr)
